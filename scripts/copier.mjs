...
const generateBaseProject = async (target, context) => {
  ...
  await writeFile(target, 'pnpm-workspace.yaml', 'packages:\n  - apps/*\n  - libs/*\n  - tools/*\n');
  await writeFile(
    target,
    'justfile',
    [
  ...
  await writeFile(target, '.github/copilot-instructions.md', '# GitHub Copilot Instructions\n\nFollow organizational guardrails.\n');
  await writeFile(target, '.github/models.yaml', 'models:\n  - name: gpt-4.1-mini\n');

  await writeFile(target, '.github/instructions/commit-msg.instructions.md', '# Commit Message Guidelines\n\nAdhere to Conventional Commits.\n');
  await writeFile(target, '.github/prompts/spec.feature.template.md', '# Spec Feature Template\n\n- Ensure acceptance criteria map to AI specs.\n');
  await writeFile(target, '.github/prompts/tdd.workflow.prompt.md', '# TDD Workflow Prompt\n\nDescribe the red/green/refactor steps.\n');
  await writeFile(target, '.github/chatmodes/default.md', '# Default Chat Mode\n\nThis mode guides collaborative development.\n');

  await writeFile(target, 'docs/README.md', '# Project Documentation\n\nThis project documents AI-enhanced development workflows. Refer to spec_index.md and commit_message_guidelines.md.\n');
  await writeFile(target, 'docs/spec_index.md', '# Specification Index\n\n- AI specifications\n');
  await writeFile(target, 'docs/traceability_matrix.md', '# Traceability Matrix\n\n| Requirement | Status |\n|-------------|--------|\n| AI_ADR-005 | Stable |\n');
  await writeFile(target, 'docs/commit_message_guidelines.md', '# Commit Message Guidelines\n\nSee .github/instructions/commit-msg.instructions.md for detailed guardrails.\n');
  await writeFile(target, 'docs/how-to/ai-onboarding.md', '# AI Onboarding Guide\n\nFollow this guide to link back to the traceability matrix and Commit Message Guidelines.\n');

  await writeFile(target, 'scripts/bundle-context.sh', '#!/usr/bin/env bash\nset -euo pipefail\necho "Bundling AI context"\n', { mode: 0o755 });
  await writeFile(target, 'tools/spec/matrix.js', 'console.log("Generating specification matrix");\n');

  await writeFile(target, 'Dockerfile', 'FROM gcr.io/distroless/cc\nUSER 65532:65532\nCMD ["/app/start"]\n');
  await writeFile(target, 'docker-compose.yml', 'services:\n  app:\n    security_opt:\n      - no-new-privileges:true\n    cap_drop:\n      - ALL\n');
};
...
const generateSecurityEnhancements = async (target, context) => {
  ...
  await writeFile(target, 'libs/security/src/secure_db.rs', `pub fn encrypt(data: &[u8]) -> Vec<u8> {\n    data.to_vec()\n}\n`);
  await writeFile(target, 'docs/security/ENCRYPTION.md', '# Encryption Overview\n\nThe system uses XChaCha20-Poly1305 with key rotation strategies.\n');
};
...
const generateAiAssets = async (target, context) => {
  if (!context.includeAiWorkflows) {
    ...
    await fs.rm(path.join(target, 'tools/temporal-db'), { recursive: true, force: true }).catch(() => {});
    await fs.rm(path.join(target, 'temporal_db'), { recursive: true, force: true }).catch(() => {});
    await fs.rm(path.join(target, '.github/workflows/ai-generate.yml'), { force: true }).catch(() => {});
    return;
  }

  await writeFile(target, '.github/workflows/ai-generate.yml', buildAiWorkflow());
  await writeFile(target, 'tools/ai/context-manager.ts', `export const refreshContext = () => {\n  console.log('Updating temporal context for AI workflows');\n};\n`);
  await writeFile(target, 'tools/ai/config.yml', 'workflows:\n  - context-management\n');
  await writeFile(target, 'tools/ai/workflows.json', JSON.stringify({ enabled: true, workflows: ['context-management', 'pattern-learning'] }, null, 2) + '\n');
  await writeFile(target, 'tools/temporal-db/init.py', 'print("initializing temporal-db")\n');
  await writeFile(target, 'temporal_db/README.md', '# Temporal Database\n\nHolds embeddings for AI workflows.\n');
};
...
const main = async () => {
  ...
  const context = {
    ...
    projectSlug: slugify(config.data.project_slug ?? dataFileValues.project_slug ?? dataFileValues.project_name ?? config.data.project_name ?? 'my-project'),
    includeAiWorkflows: toBoolean(config.data.include_ai_workflows ?? dataFileValues.include_ai_workflows, true),
    enableSecurityHardening: toBoolean(
      config.data.enable_security_hardening ??
        (dataFileValues.enable_security_hardening ?? process.env.COPIER_ENABLE_SECURITY_HARDENING),
      false
    ),
  };
