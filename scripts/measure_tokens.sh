...
legacy_count() {
    ...
    echo -e "${BLUE}[Legacy Mode] Token Analysis for ${prompt_file}:${NC}"
    echo -e "  📊 Approximate word count: ${GREEN}${word_count}${NC}"
    echo -e "  📊 Character count: ${GREEN}${char_count}${NC}"
    echo -e "  🤖 Estimated tokens (rough): ${GREEN}$((word_count * 4 / 3))${NC}"
    echo ""
    echo -e "${YELLOW}💡 For accurate token counting and optimization, install Python dependencies${NC}"
...
main() {
    ...
    if [[ "${use_legacy}" == "true" ]]; then
        ...
        echo -e "${BLUE}🚀 Using enhanced ML-powered analysis...${NC}"
        enhanced_count "${prompt_file}" "${remaining_args[@]}"
    else
        echo -e "${YELLOW}⚠️  Falling back to legacy mode...${NC}"
        legacy_count "${prompt_file}"
    fi
}
