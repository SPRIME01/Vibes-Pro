...
async function processFile(file) {
  ...
  if (secondDash === -1)
    return { file, changed: false, reason: "no-closing-frontmatter" };
  ...
  if (!hasName) {
    ...
    if (insertIndex === -1)
      insertIndex = fmLines.findIndex((l) => /^task\s*:/.test(l));
    if (insertIndex === -1)
      insertIndex = fmLines.findIndex((l) => /^domain\s*:/.test(l));
  ...
  if (!hasTools) {
    ...
    if (insertIndex === -1)
      insertIndex = fmLines.findIndex((l) => /^model\s*:/.test(l));
    if (insertIndex === -1)
      insertIndex = fmLines.findIndex((l) => /^task\s*:/.test(l));
...
async function main() {
  ...
  if (modified.length)
    modified.forEach((m) => console.log("  +", path.relative(ROOT, m)));
  console.log("Unchanged/Skipped:", skipped.length);
  if (skipped.length && skipped.length < 50)
    skipped.forEach((s) =>
      console.log("  -", path.relative(ROOT, s.f), s.reason),
    );
}
