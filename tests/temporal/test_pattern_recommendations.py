...
def test_recommendation_generation_creates_entries():
    ...
    try:
        assert result.recommendations, "Recognizer should emit recommendations"
...
def test_retention_and_feedback_controls_confidence(tmp_path: Path):
    """Retention purge and feedback adjustments should update stored confidence."""

    repository = asyncio.run(_bootstrap_repository(str(tmp_path / "temporal")))
