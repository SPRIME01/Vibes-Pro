...
async function verifyApiClientContent(
  filePath: string,
...
async function verifyConfigContent(
  filePath: string,
...
describe("Application Generator", () => {
  ...
  describe("Next.js Applications", () => {
    it("should generate Next.js applications with proper architecture", async () => {
      ...
      const nextApiClientPath = path.join(
        result.outputPath,
        "apps/admin-portal/lib/api-client.ts",
      );
      await verifyApiClientContent(nextApiClientPath, [
        "user-management",
        "billing",
      ]);
      ...
      await verifyConfigContent(nextConfigPath, [
        "experimental",
        "appDir",
      ...
      const nextIndexPath = path.join(
        result.outputPath,
        "apps/admin-portal/pages/index.tsx",
      );
    ...
    it("should handle no domains gracefully", async () => {
      ...
      const simpleApiClientPath = path.join(
        result.outputPath,
        "apps/simple-app/lib/api-client.ts",
      );
  ...
  describe("Remix Applications", () => {
    it("should generate Remix applications with proper configuration", async () => {
      ...
      const remixConfigPath = path.join(
        result.outputPath,
        "apps/remix-app/remix.config.js",
      );
      await verifyConfigContent(remixConfigPath, [
        "appDirectory",
      ...
      const remixApiClientPath = path.join(
        result.outputPath,
        "apps/remix-app/lib/api-client.ts",
      );
      ...
      const remixIndexPath = path.join(
        result.outputPath,
        "apps/remix-app/app/routes/index.tsx",
      );
    ...
    it("should generate Remix with example pages when requested", async () => {
      ...
      expect(result.files).toContain(
        "apps/remix-with-example/app/routes/index.tsx",
      );
      expect(result.files).toContain(
        "apps/remix-with-example/app/lib/api-client.ts",
      );

      // Check API client domain integration
      const remixExampleApiClientPath = path.join(
        result.outputPath,
        "apps/remix-with-example/app/lib/api-client.ts",
      );
      await verifyApiClientContent(remixExampleApiClientPath, [
        "user-management",
        "billing",
      ]);

      // Check main page has example integration
      const remixExampleIndexPath = path.join(
        result.outputPath,
        "apps/remix-with-example/app/routes/index.tsx",
      );
  ...
  describe("Expo Applications", () => {
    it("should generate Expo mobile applications with proper configuration", async () => {
      ...
      const expoAppJsonPath = path.join(
        result.outputPath,
      ...
      const expoApiClientPath = path.join(
        result.outputPath,
        "apps/mobile-app/lib/api-client.ts",
      );
      ...
      const expoAppTsxPath = path.join(
        result.outputPath,
    ...
    it("should generate Expo with example pages when requested", async () => {
      ...
      expect(result.files).toContain(
        "apps/expo-with-example/lib/api-client.ts",
      );

      // Check API client domain integration
      const expoExampleApiClientPath = path.join(
        result.outputPath,
        "apps/expo-with-example/lib/api-client.ts",
      );
      await verifyApiClientContent(expoExampleApiClientPath, [
        "user-management",
        "billing",
      ]);

      // Check App.tsx has example integration and Supabase
      const expoExampleAppTsxPath = path.join(
        result.outputPath,
        "apps/expo-with-example/App.tsx",
      );
      ...
      if (fs.existsSync(expoApiClientPath)) {
        const expoContents = await fs.promises.readFile(
          expoApiClientPath,
          "utf-8",
        );
  ...
  describe("Multi-Framework Validation", () => {
    it("should generate all required files for different frameworks", async () => {
      ...
      for (const framework of frameworks) {
        ...
        expect(result.success).toBe(true);
        expect(result.files).toContain(
          `apps/${framework}-test/lib/api-client.ts`,
        );

        // Framework-specific files and content verification
        if (framework === "next") {
          expect(result.files).toContain(
            `apps/${framework}-test/next.config.js`,
          );
          const multiNextConfigPath = path.join(
            result.outputPath,
            `apps/${framework}-test/next.config.js`,
          );
          ...
          expect(result.files).toContain(
            `apps/${framework}-test/remix.config.js`,
          );
          const multiRemixConfigPath = path.join(
            result.outputPath,
            `apps/${framework}-test/remix.config.js`,
          );
          ...
          const multiExpoAppJsonPath = path.join(
            result.outputPath,
            `apps/${framework}-test/app.json`,
          );
