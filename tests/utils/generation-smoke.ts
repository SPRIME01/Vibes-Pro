...
const runCopier = (targetDir: string, options: CopierOptions = {}): void => {
  ...
  if (typeof options.enable_security_hardening !== "undefined") {
    env.COPIER_ENABLE_SECURITY_HARDENING = String(
      Boolean(options.enable_security_hardening),
    );
  }
  ...
  if (result.status !== 0) {
    ...
    throw new Error(
      `Copier generation failed with status ${
        result.status ?? -1
      }\nSTDOUT: ${stdout}\nSTDERR: ${stderr}`,
    );
  }
...
export const runGenerationSmokeTest = async (): Promise<SmokeTestResult> => {
  ...
  try {
    ...
    const promptsAvailable = await ensureScriptExists(
      workspaceRoot,
      "prompt:lint",
    );
    const specMatrixAvailable = await ensureScriptExists(
      workspaceRoot,
      "spec:matrix",
    );
    ...
    const promptLintResult = promptsAvailable
      ...
      : {
          command: "pnpm run prompt:lint",
          status: 1,
          stdout: "",
          stderr: "prompt:lint script missing",
        };

    const specMatrixResult = specMatrixAvailable
      ? runScript(workspaceRoot, "spec:matrix")
      : {
          command: "pnpm run spec:matrix",
          status: 1,
          stdout: "",
          stderr: "spec:matrix script missing",
        };
...
export const runCopierGeneration = async (
