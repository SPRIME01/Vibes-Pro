...
assert.strictEqual(
  lintResult.ok,
  false,
  "Linter should fail for invalid model reference",
);
assert.ok(
  lintResult.findings.some((finding) =>
    finding.includes('Model "Invalid-Model" not found'),
  ),
...
const invalidInstructionPrompt = path.join(
  tmp,
  "invalid_instruction.prompt.md",
);
fs.writeFileSync(
...
test("lint prompt file validations (self-contained)", () => {
  ...
  expect(
    lintResult2.findings.some((finding) =>
      finding.includes(
        'Instruction file "nonexistent.instructions.md" not found',
      ),
    ),
  ).toBe(true);
  ...
  expect(
    resMissingThread.findings.some((f) =>
      f.includes("Missing frontmatter field: thread"),
    ),
  ).toBe(true);
  ...
  expect(
    resMissingMatrix.findings.some((f) =>
      f.includes("Missing frontmatter field: matrix_ids"),
    ),
  ).toBe(true);
