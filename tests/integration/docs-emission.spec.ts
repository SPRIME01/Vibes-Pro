...
describe('Documentation template emission', () => {
  ...
  it('should provide AI onboarding guidance within generated documentation', async () => {
    ...
    const onboardingPath = join(generatedWorkspace, 'docs/how-to/ai-onboarding.md');
    ...
    const docsReadmePath = join(generatedWorkspace, 'docs/README.md');
    ...
    const specIndexPath = join(generatedWorkspace, 'docs/spec_index.md');
    const specIndexContent = await fs.readFile(specIndexPath, 'utf-8');
    expect(specIndexContent).toContain('Specification Index');

    const commitGuidePath = join(generatedWorkspace, 'docs/commit_message_guidelines.md');
