...
describe("Project Generation Integration", () => {
  ...
  it("should scaffold a hexagonal architecture layout", async () => {
    ...
    await expect(
      ensurePathExists(join(projectPath, "libs/core/domain")),
    ).resolves.toBe(true);
    await expect(
      ensurePathExists(join(projectPath, "libs/core/application")),
    ).resolves.toBe(true);
    await expect(
      ensurePathExists(join(projectPath, "libs/core/infrastructure")),
    ).resolves.toBe(true);
    await expect(
      ensurePathExists(join(projectPath, "libs/core/interface")),
    ).resolves.toBe(true);
  });
  ...
  it("should create basic package configuration without cookiecutter remnants", async () => {
    ...
    expect(JSON.parse(packageJson).workspaces).toEqual([
      "apps/*",
      "libs/*",
      "tools/*",
    ]);
  ...
  it("should provision AI workflow scaffolding when enabled", async () => {
    ...
    await expect(ensurePathExists(join(projectPath, "tools/ai"))).resolves.toBe(
      true,
    );
    await expect(
      ensurePathExists(join(projectPath, "temporal_db")),
    ).resolves.toBe(true);
    ...
    expect(parsedConfig.workflows).toEqual([
      "context-management",
      "pattern-learning",
    ]);
