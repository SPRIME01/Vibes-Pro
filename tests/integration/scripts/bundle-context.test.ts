...
describe('bundle-context.sh script', () => {
    ...
    beforeEach(async () => {
        ...
        await fs.mkdir(join(testWorkspace, 'architecture', 'calm'), { recursive: true });
        ...
        await fs.writeFile(
            join(testWorkspace, 'techstack.yaml'),
            'name: test-stack\nversion: 1.0.0\n'
        );
    });
    ...
    it('should collect spec files with prefix', async () => {
        ...
        const specFile = await fs.readdir(bundleOutputDir)
    ...
    it('should copy CALM architecture directory', async () => {
        ...
        const calmExists = await fs.stat(calmDir)
            .then(stats => stats.isDirectory())
            .catch(() => false);
    ...
    it('should copy techstack.yaml if present', async () => {
        ...
        const techstackExists = await fs.stat(techstackPath)
    ...
    it('should handle missing directories gracefully', async () => {
        ...
        const outputExists = await fs.stat(minimalBundleDir)
            .then(stats => stats.isDirectory())
            .catch(() => false);
