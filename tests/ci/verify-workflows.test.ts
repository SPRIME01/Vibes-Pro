...
describe('GitHub workflow propagation', () => {
  ...
  it('should define expected workflow job names', async () => {
    ...
    const nodeTestWorkflow = await fs.readFile(
      join(projectRoot, '.github/workflows/node-tests.yml'),
      'utf-8',
    );
    expect(nodeTestWorkflow).toContain('Node Tests');
    expect(nodeTestWorkflow).toContain('runs-on: ubuntu-latest');

    const specGuardWorkflow = await fs.readFile(
      join(projectRoot, '.github/workflows/spec-guard.yml'),
      'utf-8',
    );
    expect(specGuardWorkflow).toContain('Spec Guard CI');
    expect(specGuardWorkflow).toContain('just spec-guard');
    // Check for composite action usage instead of inline cache setup
    expect(specGuardWorkflow).toContain('uses: ./.github/actions/setup-node-pnpm');
    expect(specGuardWorkflow).toContain('node-version: "20"');
  });
});
