...
- All edits must be confined to: `templates/{{project_slug}}/docs/`, `templates/{{project_slug}}/README.md.j2`, tests under `tests/docs/` or `tests/integration/`, `.github/prompts/` (this file may be used), and `docs/aiassit/AI_TDD_PLAN.md`.
...
1) Create or update template files
  - Convert the following files (if present) under `templates/{{project_slug}}/docs/` or `templates/{{project_slug}}/` to Jinja2 templates with `.j2` suffix:
  ...
  - Replace the following tokens in the templates using Copier's Jinja2 syntax:
    - project slug/name tokens: use `{{ project_slug }}` and `{{ project_name }}`
    - author: `{{ author_name }}`
    - year: `{{ year }}`
    - repo url: `{{ repo_url }}`
...
- `templates/{{project_slug}}/README.md.j2` and at least the four docs templates exist and contain the Copier tokens
