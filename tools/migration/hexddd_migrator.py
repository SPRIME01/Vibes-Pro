...
class HexDDDMigrator:
    ...
    def _convert_generator_file(self, generator: GeneratorInfo, template_dir: Path) -> None:
        ...
        try:
            ...
            # 1) Find all functions (both exported and non-exported)
            template_literal_map: Dict[str, str] = {}
            template_literals_raw: List[str] = []
            try:
                # Enhanced function detection patterns to capture more function types
                function_patterns = [
                    ...
                    r'(\w+)\s*\(([^)]*)\)\s*{',  # name(params) {
                    r'(\w+)\s*:\s*\(([^)]*)\)\s*=>',  # name: (params) =>
                    r'private\s+(\w+)\s*\(([^)]*)\)',  # private name(params)
                    r'public\s+(\w+)\s*\(([^)]*)\)',  # public name(params)
                    r'protected\s+(\w+)\s*\(([^)]*)\)',  # protected name(params)
                    r'static\s+(\w+)\s*\(([^)]*)\)',  # static name(params)
                    r'async\s+function\s+(\w+)\s*\(([^)]*)\)',  # async function name(params)
                    r'async\s+(\w+)\s*=\s*\(([^)]*)\)\s*=>',  # async name = (params) =>
                    r'validateOptions\s*\(([^)]*)\)',  # Special case for validateOptions function
                ]
                ...
                for pattern in function_patterns:
                    ...
                    for match in matches:
                        if isinstance(match, tuple):
                            ...
                            # Handle single match cases (like validateOptions)
                            func_name = match.strip()
                            params = ""

                        # Clean up parameter list
                        params = re.sub(r'\s+', ' ', params)  # normalize whitespace
                        params = params.replace(', ', ',')  # remove space after comma for consistency
                ...
                funcs = unique_functions
                logger.debug(f"Found {len(funcs)} functions in generator: {[f[0] for f in functions]}")

                # Special handling for validateOptions - ensure it's captured if present
                if any('validateOptions' in normalized_content for func in functions):
                    logger.debug("validateOptions function detected in generator")
            except Exception as e:
            ...
            try:
                write_ops = self._extract_write_operations(normalized_content)
                logger.debug(f"Found {len(write_ops)} write operations using structured parsing")

                if not write_ops:
                    legacy_regex = r'(?:fs\.|)writeFileSync\s*\(\s*([^,\)]+)\s*,\s*([^,\)]+)\s*\)'
                    write_ops = re.findall(legacy_regex, normalized_content)
                    logger.debug(f"Structured parsing found none; fallback regex located {len(write_ops)} operations")
            except Exception as e:
            ...
            try:
                def _add_template_literal(raw_literal: str) -> None:
                    literal = raw_literal
                    if literal and literal not in template_literals_raw:
                        template_literals_raw.append(literal)

                assignment_templates = self._extract_template_assignments(normalized_content)
                for var_name, tpl in assignment_templates.items():
                    template_literal_map[var_name] = tpl
                    _add_template_literal(tpl)
                ...
                for fname_tpl, content_tpl in write_template_pattern.findall(normalized_content):
                    _add_template_literal(fname_tpl)
                    _add_template_literal(content_tpl)

                if not template_literals_raw and '${selector}' in normalized_content:
                    template_literals_raw.append('${selector}')

            except Exception as e:
                logger.warning(f"Failed to parse template literals from {generator.name}: {e}")
                template_literals_raw = []
            ...
            for var_name, raw_tpl in template_literal_map.items():
                tpl_conv = _ts_template_to_py(raw_tpl)
                try:
            ...
            template_literals = []
            seen_templates = set()
            for raw_tpl in template_literals_raw:
                tpl_conv = _ts_template_to_py(raw_tpl)
                try:
                    tpl_conv = self._convert_template_syntax(tpl_conv)
                except Exception:
                ...
                if tpl_conv not in seen_templates:
                    ...
                    template_literals.append(tpl_conv)
            ...
            if write_ops:
                ...
                for idx, (fname_expr, content_expr) in enumerate(write_ops):
                    try:
                        ...
                        literal_fname = self._strip_wrapping_quotes(fname)
                        ...
                        literal_content = None if mapped_content is not None else self._strip_wrapping_quotes(content_expr_str)

                        if mapped_content is not None:
                            converted = mapped_content
                        elif literal_content is not None:
                            content_py = _ts_template_to_py(literal_content)
                            try:
...
'''
    ...
    def _extract_write_operations(self, content: str) -> List[Tuple[str, str]]:
        """Extract writeFileSync call arguments while respecting nested expressions."""
        operations: List[Tuple[str, str]] = []
        pattern = re.compile(r'(?:fs\.)?writeFileSync\s*\(', re.MULTILINE)
        search_pos = 0
    ...
    def _extract_template_assignments(self, content: str) -> Dict[str, str]:
        ...
        for match in pattern.finditer(content):
            ...
            literal, end_pos = self._extract_string_literal(content, match.end())
            if literal is None:
                continue
    ...
    @staticmethod
    def _strip_wrapping_quotes(expr: str) -> Optional[str]:
  ...
  constructor(
    ...
    def _convert_code_to_template(self, content: str, original_domain_name: str) -> str:
        ...
        try:
            ...
            # Use negative lookahead to avoid utility classes like ConfigService, EntityFactory, etc.
            content = re.sub(
                r'^(\s*)export\s+class\s+((?!Config|Database|Entity|Service|Repository|Locator|Manager|Factory)[A-Z][a-zA-Z]*)(Entity)(\s+|{)',
                r'\1export class {{entity_name}}Entity\4',
                content,
                flags=re.MULTILINE
            )
            entity_replacements = len(re.findall(r'^(\s*)export\s+class\s+((?!Config|Database|Entity|Service|Repository|Locator|Manager|Factory)[A-Z][a-zA-Z]*)(Entity)(\s+|{)', original_content, flags=re.MULTILINE))
        ...
        try:
            # Replace service class names - only class declarations, not references
            # Use negative lookahead to avoid utility classes like ConfigService, ServiceLocator, etc.
            content = re.sub(
                r'^(\s*)export\s+class\s+((?!Config|Database|Entity|Service|Repository|Locator|Manager|Factory)[A-Z][a-zA-Z]*)(Service)(\s+|{)',
                r'\1export class {{entity_name}}Service\4',
                content,
                flags=re.MULTILINE
            )
            service_replacements = len(re.findall(r'^(\s*)export\s+class\s+((?!Config|Database|Entity|Service|Repository|Locator|Manager|Factory)[A-Z][a-zA-Z]*)(Service)(\s+|{)', original_content, flags=re.MULTILINE))
        ...
        try:
            # Replace repository class names - only class declarations, not references
            # Use negative lookahead to avoid utility classes like RepositoryManager, etc.
            content = re.sub(
                r'^(\s*)export\s+class\s+((?!Config|Database|Entity|Service|Repository|Locator|Manager|Factory)[A-Z][a-zA-Z]*)(Repository)(\s+|{)',
                r'\1export class {{entity_name}}Repository\4',
                content,
                flags=re.MULTILINE
            )
            repo_replacements = len(re.findall(r'^(\s*)export\s+class\s+((?!Config|Database|Entity|Service|Repository|Locator|Manager|Factory)[A-Z][a-zA-Z]*)(Repository)(\s+|{)', original_content, flags=re.MULTILINE))
        ...
        try:
            # Replace import paths for entity-related files - only the path part
            # Use a more precise pattern that only matches the path, not the imported names
            def replace_import_path(match):
                full_match = match.group(0)
                # Preserve the imported names, only replace the path
                if 'from' in full_match:
                    return re.sub(
                        r"from\s+['\"](\./)[^'\"]*(entity|service|repository)[^'\"]*['\"]",
                        r"from './{{entity_name}}'",
                        full_match,
                        flags=re.IGNORECASE
                    )
                else:
                    # For import statements, keep the imported names as they are
                    return re.sub(
                        r"import\s+[^from]+from\s+['\"](\./)[^'\"]*(entity|service|repository)[^'\"]*['\"]",
                        r"import { \1 } from './{{entity_name}}'",
                        full_match,
                        flags=re.IGNORECASE
                    )

            # Match both from and import patterns
            content = re.sub(
                r"(from|import\s+[^from]+from)\s+['\"]\./[^'\"]*(entity|service|repository)[^'\"]*['\"]",
                replace_import_path,
                content,
                flags=re.IGNORECASE
            )

            original_imports = len(re.findall(r"from\s+['\"]\./[^'\"]*(entity|service|repository)[^'\"]*['\"]", original_content, flags=re.IGNORECASE))
            if original_imports > 0:
                logger.debug(f"Replaced {original_imports} entity-related import paths")
        except Exception as e:
