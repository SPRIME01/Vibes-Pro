...
async function main() {
  ...
  if (args.length < 3) {
    console.error(
      JSON.stringify({ success: false, error: "insufficient args" }),
    );
    process.exit(2);
  }
  ...
  try {
    ...
    console.error(
      JSON.stringify({
        success: false,
        error: "failed to read context file: " + String(e),
      }),
    );
    process.exit(2);
  }
  ...
  try {
    ...
    try {
      ...
      console.error(
        JSON.stringify({
          success: false,
          error: "failed to load generator module: " + String(err),
        }),
      );
      process.exit(1);
    }
  ...
  const DocumentationGenerator =
    mod.DocumentationGenerator ||
    (mod.default && mod.default.DocumentationGenerator) ||
    mod.default;
  ...
  if (!DocumentationGenerator) {
    console.error(
      JSON.stringify({
        success: false,
        error: "DocumentationGenerator not found in module",
      }),
    );
    process.exit(1);
  }
