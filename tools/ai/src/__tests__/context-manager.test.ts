...
describe("AIContextManager", () => {
  ...
  it("should provide optimal context within token budget", async () => {
    ...
    expect(context.sources.map((source) => source.id)).toContain(
      "domain-user-guide",
    );
    expect(context.sources.map((source) => source.id)).not.toContain(
      "irrelevant-notes",
    );
  });
  ...
  it("should reuse cached context for identical tasks", async () => {
    ...
    manager.registerSource(
      buildSource({
        ...
        getContent: async () =>
          "Previous domain decisions related to user entity evolution.",
      }),
    );
