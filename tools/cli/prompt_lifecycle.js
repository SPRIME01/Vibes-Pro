...
function runLifecycle(promptFile, opts = {}) {
  ...
  if (!plan.budget.within) {
    throw new Error(
      `Budget exceeded (${plan.tokens} tokens, level=${plan.budget.level})`,
    );
  }
  ...
  const res = spawnSync(
    "bash",
    ["-lc", `./scripts/run_prompt.sh "${promptFile}"`],
    {
      cwd: process.cwd(),
      stdio: "inherit",
      env: process.env,
  ...
  appendTranscript({
    label: path.basename(promptFile),
    tokens: plan.tokens,
    latencyMs: ms,
    variant: mode,
  });
...
if (require.main === module) {
  ...
  if (!file) {
    console.error(
      "Usage: node tools/cli/prompt_lifecycle.js <prompt-file> [mode]",
    );
    process.exit(2);
  }
  try {
    ...
    console.log(
      `[lifecycle] Done tokens=${out.tokens} latencyMs=${out.latencyMs}`,
    );
  } catch (e) {
